...
def launch(args):
    ...
    encoder_params = nnx.All(nnx.Param, nnx.PathContains("encoder"))
    optimizer = nnx.Optimizer(model, tx, wrt=encoder_params)
    ...
    def train_step(
        ...
        p_1 = nnx.softmax(l_1, axis=-1)

        # Define loss function with external key
        def loss_fn(model: CategoricalFlow):
            # Pass key to get_loss for random sampling
            loss = model.get_loss(loss_key, p_1, batch["image"])
            return loss

        # Compute gradient using nnx.value_and_grad
        diff_state = nnx.DiffState(0, encoder_params)
    ...
    @nnx.jit
    def eval_step(key, model: CategoricalFlow, metrics: nnx.MultiMetric, batch):
        probs = model.sample(
            key, args.num_edfm_samples, args.sample_num_steps, batch["image"]
        )
        ens_logprobs = jnp.log(jnp.mean(probs, axis=1))
...
def main():
    ...
    parser.add_argument("--num_edfm_samples", default=32, type=int)
    parser.add_argument("--sample_num_steps", default=7, type=int)
    parser.add_argument("--mixup_alpha", default=0.4, type=float)
    parser.add_argument("--exp_name", default=None, type=str)
    parser.add_argument("--save_dir", default="./checkpoint/sfm", type=str)
    parser.add_argument(
        "--resnet_dir",
        default="./checkpoint/resnet/2025-08-19_11-19-13",
        type=str,
    )
    parser.add_argument(
